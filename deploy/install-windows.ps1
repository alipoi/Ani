# Windows local machine: register a scheduled task running --fast + details every 2 minutes
...
$action = New-ScheduledTaskAction -Execute $node -Argument "`"$dir\fetch_resources.js`" --fast --details --details-max 100" -WorkingDirectory $dir
